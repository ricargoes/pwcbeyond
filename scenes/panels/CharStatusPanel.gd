...
onready var salud_node = find_node("Salud")
onready var cordura_node = find_node("Cordura")
onready var vigor_node = find_node("Vigor")
onready var voluntad_node = find_node("Voluntad")
onready var coherencia_node = find_node("Coherencia")

func _ready():
	CharactersGetter.connect("cache_refreshed", self, "refresh_ui")
...
func refresh_ui(_characters_refreshed):
	if not character:
		find_node("Name").text = "Sin personaje"
		find_node("DataBox").hide()
		return
	else:
		find_node("DataBox").show()
		character = CharactersGetter.get_character(character.chronicle, character.player_name, character.char_name)
		find_node("Name").text = character.char_name + " [" + character.player_name + "]"
...
func _on_Name_clicked(event):
	if event is InputEventMouseButton and event.button_index == BUTTON_LEFT and event.pressed:
		if character:
			var full_character_sheet = load("res://scenes/sheet_layouts/WebSheetLayout.tscn").instance()
			full_character_sheet.character = character
			var exit_button = full_character_sheet.find_node("Exit")
			exit_button.disconnect("pressed", full_character_sheet, "_on_Exit_pressed")
			exit_button.connect("pressed", self, "_on_CharSheetExit_pressed")
			full_character_sheet.find_node("AutosaveCheck").pressed = false
			full_character_sheet.find_node("Delete").queue_free()
			full_character_sheet.find_node("EditL").queue_free()
			full_character_sheet.find_node("EditCheck").queue_free()
